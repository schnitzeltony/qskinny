...
!equals( OUT_PWD, $${PWD} ) {
    ...
    rccgen.commands += && $$dirname(QMAKE_QMAKE)/rcc-qt5 $${QRC_SHADOW_CLONE} -o ${QMAKE_FILE_OUT}
    rccgen.commands += && $${QMAKE_DEL_FILE} $${QRC_SHADOW_CLONE}
}
else {
    rccgen.commands += && $$dirname(QMAKE_QMAKE)/rcc-qt5 ${QMAKE_FILE_IN} -o ${QMAKE_FILE_OUT}
}
